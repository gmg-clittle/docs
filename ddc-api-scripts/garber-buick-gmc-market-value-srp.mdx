...
1. **Script loads on SRP** → Listens to inventory events from Dealer.com API.
...
## 🧩 Script (Client-side Injection)

This script should be injected via Dealer.com’s `page-load-v1` and `vehicle-data-updated-v1` event system.
...
> ✅ **View full script in** [`vehicle-market-value-injector.js`](./vehicle-market-value-injector.js)
...
|-----------|----------|----------------------------------------------|
| `vin`     | ✅ Yes   | Comma-separated VINs to fetch MSRP for       |
| `debug`   | ❌ No    | If true, includes Redis and filtering details |

---

## 🧠 Serverless API Logic
...
> ✅ **View full handler in** [`srp-garber-buick-gmc-market-value.ts`](./srp-garber-buick-gmc-market-value.ts)
...
- [`vehicle-market-value-injector.js`](./vehicle-market-value-injector.js) – Client-side script to inject MSRP/savings
- [`srp-garber-buick-gmc-market-value.ts`](./srp-garber-buick-gmc-market-value.ts) – Serverless function
- [`redis.ts`](./lib/redis.ts) – Upstash Redis instance helper
- [`cors.ts`](./lib/cors.ts) – Edge-compatible CORS utility

---

Need help scaling this to multiple dealer rooftops or brands?  
📩 [developer-support@example.com](mailto:developer-support@example.com)
