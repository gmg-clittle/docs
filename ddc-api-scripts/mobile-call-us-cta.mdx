# GMG Mobile “Call Us” CTA – Technical Documentation
...
2. **DDC Website ID & Time Zone Resolution**
   - Uses domain → DDC ID mapping.
   - Uses domain → time zone mapping:
...
3. **Operating Hours Check**
   ...
   - Prioritizes:
     1. `SALES` hours
     2. `DEALERSHIP` hours
     3. Default fallback: **8am–6pm local time**
   - If closed → CTA NOT shown.
1. **Phone Number Resolution**
   - Tries to extract phone from DNI cookie (`ai-dni`).
   - If unavailable → use domain-specific fallback phone.
   - If still empty → no CTA injected.
2. **CTA Injection**
...
If any fail → CTA is not inserted.
...
- Uses dealership’s local time via `Intl.DateTimeFormat`
...
👉 [**https://garberspecials.com/support**](https://garberspecials.com/support)

---

## Summary

This script provides a production‑grade, time‑zone‑aware, DNI‑aware mobile Call Us button that:
...
- Integrates cleanly with Dealer.com’s Web Integration API

Maintained and updated by\
**Garber Digital Marketing Team**\
_Last Edited: December 5, 2025 — Chris Little_