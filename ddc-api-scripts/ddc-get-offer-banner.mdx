...
This document describes how the **Universal SRP Get-Offer Banner** script works on [Dealer.com](http://Dealer.com) Search Results Pages (SRP).\\
...
\<Steps\>
  ...
  \<Step title="Hook into [Dealer.com](http://Dealer.com) events"\>
...
This script is designed to run inside the [**Dealer.com**](http://Dealer.com)** Web Integration API** environment. It should be loaded via the DDC `APILoader` pattern.

:::

**Requires:**

- `window.DDC.APILoader` ([Dealer.com](http://Dealer.com) Web Integration API)
...
\<Cards\>
  ...
  \<Card title="[Dealer.com](http://Dealer.com) Web Integration API"\>
...
- `webhookURL` – [Make.com](http://Make.com) (or other) webhook receiving form submissions.
...
window.GETOFFER_CONFIG = {
  ...
  overrides: {
    ...
    domain: {
      ...
      '[garbermidland.com](http://garbermidland.com)': {
...
If `restrictToNew` is `true`, the banner only renders when `inventoryType === 'new'`.
...
if (event.origin === origin && [event.data](http://event.data) === 'formSubmissionSuccess') {
...
The script is designed to **fail gracefully**: if any step fails (API error, no offers, bad image URL), the banner is collapsed and SRP layout is preserved.
...
\<Steps\>
  ...
  \<Step title="Add script to DDC Web Integration"\>
    ...
    Include the compiled JS file in the [Dealer.com](http://Dealer.com) Web Integration admin, scoped to SRP templates only if possible.
  ...
  \<Step title="QA on one store"\>
    ...
    Turn on \<code\>debug: true\</code\>, confirm:

    \<ul\>

      \<li\>Correct model is detected.\</li\>

      \<li\>Banner appears only when an active special exists.\</li\>

      \<li\>Click opens the iframe with correct pre-filled values.\</li\>
...
For questions, feature requests, or issues with this banner, please submit a ticket at\\
